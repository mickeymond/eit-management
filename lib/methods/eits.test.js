...
if (Meteor.isServer) {
  describe('EITs', () => {
    describe('methods', () => {
      ...
      it('Can view EITs', () => {
        dummyEITs.map(dummyEIT => EITs.insert({ ...dummyEIT, mentor: loggedInUser._id }));
        const eitsPublication = Meteor.server.publish_handlers['eits'];

        const invocation = {};
        
        const tasks = eitsPublication.apply(invocation);

        assert.equal(tasks.count(), 1 + dummyEITs.length);
      });
      ...
      it('Cannot delete someone\'s EIT', () => {
        const deleteEIT = Meteor.server.method_handlers['eits.delete'];
      ...
      it('Cannot bulk delete someone\'s EITs', () => {
        // Insert Bulk EITs before test
        const ids = dummyEITs.map(dummyEIT => EITs.insert({ ...dummyEIT, mentor: loggedInUser._id }));
